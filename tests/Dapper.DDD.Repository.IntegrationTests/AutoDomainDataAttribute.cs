	...
	public AutoDomainDataAttribute([CallerMemberName] string callerMemberName = "") : base(() =>
	  {
		  ...
		  fixture.Customize<Customer>(transform => transform
														...
														.With(customer => customer.InvoiceAddress, new Address { Street = "Streetname" + Guid.NewGuid(), Zipcode = new Zipcode(Random.Shared.Next(int.MaxValue)) })
														.With(customer => customer.DeliveryAddress, new Address { Street = "Streetname" + Guid.NewGuid(), Zipcode = new Zipcode(Random.Shared.Next(int.MaxValue)) }));
		  return fixture;
	  })
	{
	}
