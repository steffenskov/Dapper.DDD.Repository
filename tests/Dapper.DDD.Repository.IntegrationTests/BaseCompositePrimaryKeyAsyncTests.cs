...
public abstract class BaseCompositePrimaryKeyAsyncTests : BaseTests
{
	...
	{
		// Act
		var deleted = await _repository.DeleteAsync(new CompositeUserId("async My name", "Secret"), TestContext.Current.CancellationToken);
	...
	public async Task Delete_UsePrimaryKey_Valid(CompositeUser aggregate)
	{
		...
		// Act
		var deleted =
			await _repository.DeleteAsync(new CompositeUserId(aggregate.Id.Username, aggregate.Id.Password), TestContext.Current.CancellationToken);
	...
	{
		// Act
		var gotten = await _repository.GetAsync(new CompositeUserId("async My name", "Secret"), TestContext.Current.CancellationToken);
	...
	public async Task Get_UsePrimaryKey_Valid(CompositeUser aggregate)
	{
		...
		// Act
		var gotten = await _repository.GetAsync(aggregate.Id, TestContext.Current.CancellationToken);
		...
		await _repository.DeleteAsync(insertedAggregate.Id, TestContext.Current.CancellationToken);
	}
	...
	public async Task Update_UsePrimaryKey_Valid(CompositeUser aggregate)
	{
		...
		// Act
		var updated = await _repository.UpdateAsync(insertedAggregate with { Age = 42 }, TestContext.Current.CancellationToken);
