...
internal static class ConstructorBuilder
{
	...
	{
		var assemblyBuilder =
			AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("Dynamic"), AssemblyBuilderAccess.Run);
		...
		var constructorBuilder = typeBuilder.DefineConstructor(
			...
			MethodAttributes.SpecialName, CallingConventions.Standard, Type.EmptyTypes);
		var ilGenerator = constructorBuilder.GetILGenerator();
		ilGenerator.Emit(OpCodes.Ret);

		var generatedType = typeBuilder.CreateType();
		var ctorInfo = generatedType!.GetConstructor(
